      ...
      <el-table-column label="利润" prop="profit">
        <!-- <template slot-scope="{ row }">
          <span style="color: red">
            {{
              (
                (parseFloat(row.sellPrice) - parseFloat(row.buyPrice)) *
                parseInt(row.sellShares)
              ).toFixed(2) || 0
            }}
          </span>
        </template> -->
  ...
  data() {
    return {
      ...
      listQuery: {
        ...
        limit: 20,
        dataType: undefined,
        referencePrice: 0.879,
        buyThreshold: 0.9,
        sellThreshold: 1.1111,
        minPrice: 0.1,
        maxPrice: 5.7,
        buyAmount: 10000,
        minStockUnit: 100,
        ...
        this.list.forEach((item) => {
          item.profit =
            (
              ...
              parseInt(item.sellShares)
            ).toFixed(2) || 0
          if (isNaN(item.profit)) {
            item.profit = ''
          }
        })
